<script lang="ts">
    ...
    const modificaLayout = () =>{
        swm = 1;
        codiceSpecchio=codice1;
        descrizioneArticoloSpecchio=descrizione_articolo;
        prezzoSpecchio=prezzo;
        quantitaSpecchio=quantita;
...
if (result.success) { 
    ...
    <div class="container" in:fly={{ y: 20, duration: 450 }}>
        ...
        <div class="ricerca" in:fly={{ x: -50, duration: 400 }} out:fly={{ x: 50, duration: 400 }}>
            <input  type="text" class="lente" bind:value={codice} oninput={controlla} class:sw2={sw === 2||sw === 3} >
        ...
        {#if sw === 1}
            <div class="table-container" in:fade={{ duration: 500 }} out:fade={{ duration: 500 }}>
                ...
                {#if swm==0}
                <!-- svelte-ignore a11y_consider_explicit_label -->
                <button class="svg" in:fly={{ y: -20, duration: 400 }} 
                onclick={modificaLayout}> <svg width="25px" height="25px" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" stroke="#247eca">
                ...
                <!-- svelte-ignore a11y_consider_explicit_label -->
                <button class="svg" in:fly={{ y: -20, duration: 400 }} 
                onclick={() => modifica(codiceSpecchio)}><svg width="20px" height="20px" viewBox="0 0 1024 1024" class="icon" version="1.1" xmlns="http://www.w3.org/2000/svg" fill="#247eca" stroke="#247eca">
                ...
                <!-- svelte-ignore a11y_consider_explicit_label -->
                <button class="svg" in:fly={{ y: -20, duration: 400 }} 
                onclick={annullaModifica}><svg width="20px" height="20px" viewBox="0 0 1024 1024" class="icon" version="1.1" xmlns="http://www.w3.org/2000/svg" fill="#247eca" stroke="#247eca">
